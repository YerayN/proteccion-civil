document.getElementById('informe-form').addEventListener('submit', async function (event) {
    ...
    y = addTextWithWrapAndStyle(`- Título: `, titulo, margin, y, 'bold');
    y = addTextWithWrapAndStyle(`- Sugerencia: `, sugerencia, margin, y, 'bold');
    ...
    try {
        const response = await fetch('/api/upload_sugerencias', {
            method: 'POST',
            body: formData
        });
